...
#define CFG_HAVE_SERIAL_H  1

#ifdef CFG_HAVE_SERIAL_H
#include <linux/serial.h> /* struct serial_struct  */
#else
#warning "no linux/serial.h, use local definition of serial_struct!"
...
#define  WMT_PLAT_APEX  1  //add by dg for debug linux

#ifndef WMT_PLAT_APEX
#define  WCN_COMBO_CHIP_ID_PROP    "persist.mtk.wcn.combo.chipid"
#else
#define  WCN_COMBO_CHIP_ID_PROP    "persist.radio.mediatek.chipid"
#endif
...
int cmd_hdr_stp_open (P_STP_PARAMS_CONFIG pStpParamsConfig) {
    ...
    if ((STP_UART_FULL == gStpMode) && (-1 == gTtyFd)) {
        ...
        if (gTtyFd < 0) {
            fprintf(stderr, "Can't open serial port %s,error:%s\n", gStpDev,strerror(errno));
            return -2;
        }
        printf("real_tty(%s) opened(%d)\n", gStpDev, gTtyFd);

        if ((ioctl(gTtyFd, TIOCGSERIAL, &ss)) < 0) {
            printf("%s: BAUD: error to get the serial_struct info:%s\n", __FUNCTION__, strerror(errno));
            return -4;
        }
        printf("%s: get ss.flags:0x%08x\n", __FUNCTION__, ss.flags);


#if 0
        ss.flags |= ASYNC_LOW_LATENCY;
        /* set uart low latency */
        if ((ioctl(gTtyFd, TIOCSSERIAL, &ss)) < 0) {
            printf("%s: BAUD: error to set serial_struct:%s\n", __FUNCTION__, strerror(errno));
            return -5;
        }
#endif
...
#if 0  //add by dg
        /* from the stp_uart driver log, we need to re-open 2nd time to let it
        * take effect.
        */
        printf("real_tty(%s) closed(%d)\n", gStpDev, gTtyFd);
        close(gTtyFd);
        gTtyFd = open(gStpDev, O_RDWR | O_NOCTTY);
        if (gTtyFd < 0) {
            fprintf(stderr, "Can't open serial port %s 2nd time\n", gStpDev);
            return -2;
        }
        printf("real_tty(%s) opened(%d)\n", gStpDev, gTtyFd);
        if ((ioctl(gTtyFd, TIOCGSERIAL, &ss)) < 0) {
        ...
        if (ss.flags & ASYNC_LOW_LATENCY) {
            printf("%s: we have ASYNC_LOW_LATENCY now\n", __FUNCTION__);
        }
#endif
#if 0//add by dg
        //printf("Set tty->low_latency\n");
...
int cmd_hdr_sch_patch (P_STP_PARAMS_CONFIG pStpParamsConfig)
{
    ...
    if (gWmtFd > 0)
    {
        ...
        if((!strcmp(chipName,"mt6620")) || (!strcmp(chipName,"mt6628")) ||
                (!strcmp(chipName,"mt6572")) || (!strcmp(chipName,"mt6582")))
        {
            if((!strcmp(chipName,"mt6572")) || (!strcmp(chipName,"mt6582")))
            {
...
static int get_wmt_cfg (int chipId)
{
    ...
    //dg add for linux
#define WMTCFGFILEPATH "/etc/firmware/WMT.cfg"
...
int main(int argc, char *argv[])
{
    ...
    if (gWmtFd < 0) {
        printf("Can't open device node(%s),err:%s \n", CUST_COMBO_WMT_DEV,strerror(errno));
        goto clean_up;
    }
    ...
    gWmtFd = open(CUST_COMBO_WMT_DEV, O_RDWR | O_NOCTTY);
    if (gWmtFd < 0) {
        printf("Can't open device node(%s),error:%s \n", CUST_COMBO_WMT_DEV,strerror(errno));
        goto clean_up;
    }
    printf("open device node succeed.(Node:%s, fd:%d) \n", CUST_COMBO_WMT_DEV, gWmtFd);

invalid_chip:
    //chipId = check_chip_id();
    ...
    ioctl(gWmtFd, WMT_IOCTL_WMT_COREDUMP_CTRL, 1);//add by dg for linux os
    ...
    ioctl(gWmtFd, WMT_IOCTL_SET_STP_MODE, ((sStpParaConfig.sUartConfig.fc & 0x3) << 30)|((baud & 0xFFFFFF) << 8) | ((gFmMode & 0x0F) << 4)  |(gStpMode & 0x0F));
    ...
    while (!__io_canceled) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            {
                fprintf(stderr, "write resp(%d) OK: len(%d), errno(%d, %s)\n", gWmtFd, len, errno, (len == -1) ? strerror(errno) : "");
            }
        }
    }
